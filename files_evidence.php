        ...
        <div class="row flex-grow-1">
            ...
            <div class="col-10 bg-light">
                ...
                <h3 class="titulo-tabla">
                    ...
                    if(($_SESSION['user_profile'] == 1 || $_SESSION['user_profile'] == 2) && $evidence_status == 1 && $project_status != 3){ // Admin and Developer only and evidence can be editted?>
                        <a href='create_file.php?evidence_id=<?=$evidence_id?><?php if (isset($_GET['project_id'])) echo "&project_id=" . $project_id; ?><?php if (isset($_GET['project_page'])) echo "&project_page=" . $project_page; ?>&evidence_page=<?= $evidence_page ?>' class='btn btn-success pull-right'>Upload File</a>
                    <?php
                    }
                    ?>
                </h3>
                ...
                $offset = ($page - 1) * 10;
                $sql = "SELECT * FROM files WHERE evidence_id=? LIMIT 10 OFFSET $offset";
                $stmt = mysqli_prepare($connection,  $sql);
                mysqli_stmt_bind_param($stmt, "i", $evidence_id);
                        ...
                        <tbody class="border border-light bg-light">
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="download_file.php?id=<?=$row['id']?>"><span class='material-icons text-success' aria-hidden='true'>download</span></a>
                                        <?php 
                                        if(($_SESSION['user_profile'] == 1 || $_SESSION['user_profile'] == 2)){ // Admin and Developer only and evidence can be editted?>
                                            <a href='delete_file.php?id=<?= $row['id'] ?>&evidence_id=<?=$evidence_id?><?php if (isset($_GET['project_id'])) echo "&project_id=" . $project_id; ?><?php if (isset($_GET['project_page'])) echo "&project_page=" . $project_page; ?>&evidence_page=<?= $page ?>&file_page=<?=$page?>' title='Delete File' data-toggle='tooltip'> <span class='material-icons' aria-hidden='true' style='color:crimson;'>delete_sweep</span></a>
                                        <?php
                                        }
                                        ?>
                                    </td>
                ...
                <?php
                    $sql = "SELECT * FROM files WHERE evidence_id=?"; // Total files in bd
                    $stmt = mysqli_prepare($connection,  $sql);
                    mysqli_stmt_bind_param($stmt, "i", $evidence_id);
                    if (mysqli_stmt_execute($stmt))
                        $result = mysqli_stmt_get_result($stmt);
                        ...
                        echo "Oops! Something went wrong. Please try again later.";
                    $numTotal   = mysqli_num_rows($result); // Total registers
                    ...
                    if (isset($_GET['project_id'])) { // Pages conditions
                        if ($page > 1) {
